...
static int32_t g_vcam_fd;
static int32_t g_sock_fd;
static pthread_t g_vcam_pid;
static int32_t g_mem_fd;
static int32_t g_mem_len;
...
void *vcam_next_msg_thread(void *data)
{
	...
	while(1) {
		ret = ioctl(g_vcam_fd, HB_VCAM_NEXT_GROUP, &next_msg);/* PRQA S 4513 */
		if(ret < 0) {
		...
		ret = (int32_t)write(g_sock_fd, &next_msg, sizeof(hb_vcam_msg_t));
		if(ret != (int32_t)sizeof(hb_vcam_msg_t)) {
...
int32_t hb_vin_vcam_next_group()
{
	...
	int32_t ret;
	ret = ioctl(g_vcam_fd, HB_VCAM_NEXT_GROUP, &next_group);/* PRQA S 4513 */	
	if(ret < 0) {
		vin_err("get next group fail\n");
	...
	ret = (int32_t)write(g_sock_fd, &next_group, sizeof(hb_vcam_msg_t));
	if(ret != (int32_t)sizeof(hb_vcam_msg_t)) {
		vin_err("hb_vin_vcam_next_group write fail ret %d\n", ret);
		return -1;
	}
...
int32_t hb_vin_vcam_get_img(cam_img_info_t *cam_img_info)
{
	...
	ret = ioctl(g_vcam_fd, HB_VCAM_GET_IMG, cam_img_info);/* PRQA S 4513 */
	if(ret < 0) {
...
int32_t hb_vin_vcam_free_img(cam_img_info_t *cam_img_info)
{
	...
	ret = ioctl(g_vcam_fd, HB_VCAM_FREE_IMG, cam_img_info);/* PRQA S 4513 */
	if(ret < 0) {
...
int32_t hb_vin_vcam_clean(cam_img_info_t *cam_img_info)
{
	...
	ret = ioctl(g_vcam_fd, HB_VCAM_CLEAN, cam_img_info);/* PRQA S 4513 */
	if(ret < 0) {
...
int32_t hb_vin_vcam_init(hb_vcam_msg_t *init_msg)
{
	...
	ret = ioctl(g_vcam_fd, HB_VCAM_INIT, init_msg);/* PRQA S 4513 */
	if (ret < 0) {
	...
	ret = (int32_t)write(g_sock_fd, init_msg, sizeof(hb_vcam_msg_t));
	if(ret != (int32_t)sizeof(hb_vcam_msg_t)) {
		vin_err("write fail ret %d\n", ret);
		return -1;
	}
	/* create vcam thread */
	...
	/* for mmap vaddr */
	ret = ioctl(g_vcam_fd, HB_VCAM_MEMINFO, &mem_info);/* PRQA S 4513 */
	if (ret < 0) {
	...
	g_vaddr_base = (uint64_t)mmap(NULL, (uint64_t)mem_info.size,
		PROT_READ|PROT_WRITE, MAP_SHARED, g_mem_fd, mem_info.base);
	return 0;
}
...
int32_t hb_vin_vcam_start()
{
	...
	vin_info("vcam start\n");
	ret = ioctl(g_vcam_fd, HB_VCAM_START, &first_group);/* PRQA S 4513 */
	if (ret < 0) {
	...
	ret = (int32_t)write(g_sock_fd, &first_group, sizeof(hb_vcam_msg_t));
	if(ret != (int32_t)sizeof(hb_vcam_msg_t)) {
...
int32_t hb_vin_vcam_stop()
{
	...
	vin_info("vcam stop %d\n", __LINE__);
	ret = ioctl(g_vcam_fd, HB_VCAM_STOP, &stop_msg);/* PRQA S 4513 */
	if (ret < 0) {
		vin_err("vcam stop ioctl fail\n");
		return -1;
	}
	stop_msg.info_type = CMD_VCAM_STOP;
	ret = (int32_t)write(g_sock_fd, &stop_msg, sizeof(hb_vcam_msg_t));
	if(ret != (int32_t)sizeof(hb_vcam_msg_t)) {
		vin_err("hb_vin_vcam_start write fail ret %d\n", ret);
	}
	vin_info("vcam stop %d\n", __LINE__);
...
int32_t hb_vin_vcam_deinit()
{
	...
	vin_info("vcam deinit %d\n", __LINE__);
	ret = ioctl(g_vcam_fd, HB_VCAM_DEINIT, &deinit_msg);/* PRQA S 4513 */	 
	if (ret < 0) {
		vin_err("vcam deinit ioctl fail\n");
	}
	deinit_msg.info_type = CMD_VCAM_DEINIT;
	ret = (int32_t)write(g_sock_fd, &deinit_msg, sizeof(hb_vcam_msg_t));// TBD need flags or not
	if(ret != (int32_t)sizeof(hb_vcam_msg_t)) {
		vin_err("hb_vin_vcam_deinit write fail ret %d\n", ret);
	}
	munmap((void *)g_vaddr_base, (uint64_t)g_mem_len);
	close(g_mem_fd);
	close(g_sock_fd);
	close(g_vcam_fd);
	vin_info("vcam deinit end %d\n", __LINE__);
	return 0;
}
