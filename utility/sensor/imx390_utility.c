...
#define GW5200_J2_DELAY_DEF  3000

int sensor_set_390_reg_setting_unvalid(int bus, int sensor_addr)
{
	int ret = RET_OK;

	ret = hb_vin_i2c_write_reg16_data8(bus, sensor_addr, 0x0008, 1);
	if(ret <0) {
		vin_err("390_reg_setting_unvalid i2c write 0x0008 error ret %d\n", ret);
		ret = -RET_ERROR;
	}
	return ret;
}

int sensor_set_390_reg_setting_valid(int bus, int sensor_addr)
{
	int ret = RET_OK;

	ret = hb_vin_i2c_write_reg16_data8(bus, sensor_addr, 0x0008, 0);
	if(ret <0) {
		vin_err("390_reg_setting_valid i2c write 0x0008 error ret %d\n", ret);
		ret = -RET_ERROR;
	}
	return ret;
}

int sensor_imx390_954_init(sensor_info_t *sensor_info)
{
	int ret = RET_OK;
	int setting_size = 0;
	...
	if(sensor_info->config_index == 1) {
		...
		ret = vin_write_array(deserial_if->bus_num, deserial_if->deserial_addr, 1, setting_size, ds954_imx390_init_setting);
		if (ret < 0) {
			vin_err("write ds954_imx390_init_setting error\n", sensor_info->serial_addr);
			return ret;
		}
		usleep(100*1000);
		vin_dbg("usleep(100*1000)\n");
		setting_size = sizeof(ds954_imx390_init_setting)/sizeof(uint32_t)/2 - 1;
		ret = vin_write_array(deserial_if->bus_num, deserial_if->deserial_addr, 1, setting_size, &(ds954_imx390_init_setting[2]));
		if (ret < 0) {
			vin_err("write ds954_imx390_init_setting[2] error\n", sensor_info->serial_addr);
			return ret;
		}
	} else if (sensor_info->config_index == 2) {
		setting_size = sizeof(ds954_imx390_alias_init_setting)/sizeof(uint32_t)/2;
		ret = vin_write_array(deserial_if->bus_num, deserial_if->deserial_addr, 1, setting_size, ds954_imx390_alias_init_setting);
		if (ret < 0) {
			vin_err("uwrite ds954_imx390_alias_init_setting error\n", sensor_info->serial_addr);
			return ret;
		}
	}
...
{
	int gpio, ret = RET_OK;
	char *gw5200_j2_delay_env = NULL;
	int gw5200_j2_delay;

	gw5200_j2_delay_env = getenv("GW5200_J2_DELAY");
	if (gw5200_j2_delay_env) {
		vin_dbg("gw5200_j2_delay_env set(%s)\n", gw5200_j2_delay_env);
		gw5200_j2_delay = atoi(gw5200_j2_delay_env);
	} else {
		vin_dbg("gw5200_j2_delay_env not set,using default(%d ms)\n", GW5200_J2_DELAY_DEF);
		gw5200_j2_delay = GW5200_J2_DELAY_DEF;
	}
...
int sensor_set_gain_sp1_h_part(int bus, int sensor_addr, struct SP1_HCG_P_gain_s sp1_hcg)
{
	...
	ret = hb_vin_i2c_write_reg16_data8(bus, sensor_addr, AGAIN_SP1H,
			sp1_hcg.AGAIN_SP1H_num & 0xff);
	ret |= hb_vin_i2c_write_reg16_data8(bus, sensor_addr, PGA_GAIN_SP1H + 1,
			(sp1_hcg.PGA_GAIN_SP1H_num >> 8 ) & 0x03);
	ret |= hb_vin_i2c_write_reg16_data8(bus, sensor_addr, PGA_GAIN_SP1H,
			sp1_hcg.PGA_GAIN_SP1H_num & 0xff);
	if(ret <0) {
...
int sensor_set_gain_sp1_l_part(int bus, int sensor_addr, struct SP1_LCG_P_gain_s sp1_lcg)
{
	...
	ret = hb_vin_i2c_write_reg16_data8(bus, sensor_addr,
			AGAIN_SP1H, sp1_lcg.AGAIN_SP1L_num & 0xff);
	ret |= hb_vin_i2c_write_reg16_data8(bus, sensor_addr,
			PGA_GAIN_SP1H + 1, (sp1_lcg.PGA_GAIN_SP1L_num >> 8 ) & 0x03);
	ret |= hb_vin_i2c_write_reg16_data8(bus, sensor_addr,
			PGA_GAIN_SP1H, sp1_lcg.PGA_GAIN_SP1L_num & 0xff);
	if(ret <0) {
...
int sensor_set_gain_sp1_h_toal(int bus, int sensor_addr, struct SP1_HCG_T_gain_s sp1_hcg)
{
	...
	ret = hb_vin_i2c_write_reg16_data8(bus, sensor_addr,
			GAIN_SP1H, sp1_hcg.GAIN_SP1H_num & 0xff);
	if(ret <0) {
...
int sensor_set_gain_sp1_l_toal(int bus, int sensor_addr, struct SP1_LCG_T_gain_s sp1_lcg)
{
	...
	ret = hb_vin_i2c_write_reg16_data8(bus, sensor_addr,
			GAIN_SP1L, sp1_lcg.GAIN_SP1L_num & 0xff);
	if(ret <0) {
...
int sensor_set_awb(int bus, int sensor_addr, float rg_gain, float b_gain)
{
	...
	awb_data.WBG_GR = 256;
	awb_data.WBG_GB = 256;
	vin_dbg("sensor_set_awb...\n");
	sensor_set_390_reg_setting_unvalid(bus, sensor_addr);
	...
	ret |= hb_vin_i2c_write_reg16_data8(bus, sensor_addr, 0x37, ((awb_data.WBG_B >> 8) & 0x0f));
	ret |= hb_vin_i2c_write_reg16_data8(bus, sensor_addr, 0x36, awb_data.WBG_B & 0xff);
	if (ret < 0) {
		vin_err("sensor_set_awb fail\n");
		return ret;
	}
	ret = sensor_set_390_reg_setting_valid(bus, sensor_addr);
	if (ret < 0) {
		vin_err("sensor_set_390_reg_setting_valid fail ret %d \n", ret);
		return ret;
	}
	return ret;
}
...
int sensor_set_sp2_exposure(int bus, int sensor_addr, uint32_t input_data)
{
	...
	if(shs2_data < 2 || shs2_data > Global_SP2_ex)
		return -RET_ERROR;
	// hb_i2c_write_block(SHS2 + 2, (shs2_data >> 16) & 0xff);
	// hb_i2c_write_block(SHS2 + 1, (shs2_data >> 8) & 0xff);
	// hb_i2c_write_block(SHS2, shs2_data & 0xff);
...
int sensor_global_data_init(int bus, int sensor_addr)
{
	...
	sensor_set_sp1_exposure(bus, sensor_addr,337);
	sensor_set_sp2_exposure(bus, sensor_addr,371);
...
int sensor_init(sensor_info_t *sensor_info)
{
	...
	ret = sensor_imx390_954_init(sensor_info);
	if (ret < 0) {
		vin_err("imx390_954_init fail\n");
		return ret;
	}
	usleep(100*1000);
	vin_info("953init...\n");
	...
	if (ret < 0) {
		vin_err("953init write 0x%x error\n", sensor_info->serial_addr);
		deserial_if = (deserial_info_t *)sensor_info->deserial_info;
	...
	if (ret < 0) {
		vin_err("imx390 write 0x3053 error\n");
		return ret;
	}
	setting_size = sizeof(imx390_calibration_setting_SHD_DIFF)/sizeof(uint32_t)/2;
	ret = vin_write_array(sensor_info->bus_num, sensor_info->sensor_addr, 2, setting_size,
				imx390_calibration_setting_SHD_DIFF);
	if (ret < 0) {
		vin_err("imx390 vin_write_array error\n");
		return ret;
	}
	temp_data = 0;
	...
	if (ret < 0) {
		vin_err("imx390 write 0x1d0 error\n");
		return ret;
	}
	temp_data = 0;
	...
	if (ret < 0) {
		vin_err("imx390 write 0x3AF6 error\n");
		return ret;
	}
	...
	if (ret < 0) {
		vin_err("imx390 write imx390_calibration_setting_SGAIN error\n");
		return ret;
	}
	temp_data = 0;
	temp_data = hb_vin_i2c_read_reg16_data8(sensor_info->bus_num, sensor_info->sensor_addr, 0x1d0);
