...
int sensor_init(sensor_info_t *sensor_info)
{
	...
	/* set ox03j10 trigger mode */
	if (sensor_info->config_index & TRIG_MODE) {
		usleep(100*1000);
		setting_size = sizeof(ox03j10_stream_off_setting) / sizeof(uint32_t) / 2;
		for (i = 0; i < setting_size; i++) {
			ret = hb_vin_i2c_write_reg16_data8(sensor_info->bus_num,
				sensor_info->sensor_addr, ox03j10_stream_off_setting[i * 2],
				ox03j10_stream_off_setting[i * 2 + 1]);
			if (ret < 0) {
		...
		if(sensor_info->config_index & TRIG_SHUTTER) {
			setting_size = sizeof(ox03j10_trigger_shutter_mode_setting) /
				sizeof(uint32_t) / 2;
			ret = vin_write_array(sensor_info->bus_num, sensor_info->sensor_addr,
				SERDES_REG_WIDTH, setting_size, ox03j10_trigger_shutter_mode_setting);
		} else if (sensor_info->config_index & TRIG_EXTERNAL) {
			setting_size = sizeof(ox03j10_trigger_external_mode_setting) /
				sizeof(uint32_t) / 2;
			ret = vin_write_array(sensor_info->bus_num, sensor_info->sensor_addr,
				SERDES_REG_WIDTH, setting_size, ox03j10_trigger_external_mode_setting);
		}
		if (ret < 0) {
		...
		setting_size = sizeof(ox03j10_stream_on_setting) / sizeof(uint32_t) / 2;
		for (i = 0; i < setting_size; i++) {
			ret = hb_vin_i2c_write_reg16_data8(sensor_info->bus_num,
				sensor_info->sensor_addr, ox03j10_stream_on_setting[i * 2],
				ox03j10_stream_on_setting[i * 2 + 1]);
			if (ret < 0) {
	...
	if (sensor_info->config_index & TEST_PATTERN) {
		...
		usleep(40 * 1000);
		setting_size = sizeof(ox03j10_pattern_mode_setting) /
				sizeof(uint32_t) / 2;
		ret = vin_write_array(sensor_info->bus_num, sensor_info->sensor_addr,
				SERDES_REG_WIDTH, setting_size, ox03j10_pattern_mode_setting);
		if (ret < 0) {
				pr_err("senor %s write ox03j10_pattern_mode_setting\n",
						sensor_info->sensor_name);
				return ret;
		}
		setting_size = sizeof(ox03j10_stream_on_setting) / sizeof(uint32_t) / 2;
	...
	if (((25 == sensor_info->fps) || (sensor_info->extra_mode == HKISX031)) &&
		...
		setting_size = sizeof(ox03j10_stream_off_setting) / sizeof(uint32_t) / 2;
		ret = vin_write_array(sensor_info->bus_num, sensor_info->sensor_addr,
			SERDES_REG_WIDTH, setting_size,	ox03j10_stream_off_setting);
		if (ret < 0) {
		...
		// 	sensor_info->sensor_addr, ox03j10_vmax_setting[0],
		// 	ox03j10_vmax_setting[1]);
		// if (ret < 0) {
		...
		usleep(200 * 1000);
		ret = hb_vin_i2c_write_reg16_data8(sensor_info->bus_num,
			sensor_info->sensor_addr, ox03j10_stream_on_setting[0],
			ox03j10_stream_on_setting[1]);
		if (ret < 0) {
...
int sensor_start(sensor_info_t *sensor_info)
{
	...
	if (sensor_info->config_index & DUAL_LANE) {
		setting_size = sizeof(ox03j10_stream_on_setting) / sizeof(uint32_t) / 2;
		ret = vin_write_array(deserial_if->bus_num, COMMON_ADDR_SENSOR,
			SERDES_REG_WIDTH, setting_size,	ox03j10_stream_on_setting);
		if (ret < 0) {
