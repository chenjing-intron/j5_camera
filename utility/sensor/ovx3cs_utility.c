...
// #ifndef NODIAG
    ...
    if (ret == 0) {
        if (diag_send_event_stat_and_env_data(DIAG_MSG_PRIO_MID,
                ...
                NULL, 0) < 0) {
            vin_err("mod id 0x%x event id 0x%x success send failed\n", mod_id, event_id);
        }
    }
    if (ret != 0) {
        if (diag_send_event_stat_and_env_data(DIAG_MSG_PRIO_MID,
                ...
                (uint8_t *)&ret, 4) < 0) {
            vin_err("mod id 0x%x event id 0x%x fail send failed\n", mod_id, event_id);
        }
    }
#endif
	return 0;
			...
			int32_t sensor_addr, uint8_t *pdata, int32_t setting_size)
{
	...
	for (i = 0; i < setting_size;) {
		...
		if (len == 5) {
			...
			// 	usleep(5*1000);
			i = i + len + 1;
			vin_info("write ovx3c %d@0x%02x: 0x%04x=0x%04x\n", bus, i2c_slave, reg_addr, value);
		} else if (len == 4) {
			i2c_slave = pdata[i + 1] >> 1;
			reg_addr = (pdata[i + 2] << 8) | pdata[i + 3];
			value = pdata[i + 4];
			...
			sensor_turning_data_t *turning_data)
{
	...
	if ((sensor_info->extra_mode & EXT_MODE) == GAX3C ||
		(sensor_info->extra_mode & EXT_MODE) == LCEX3C_60 ||
		(sensor_info->extra_mode & EXT_MODE) == LCEX3C_100 ||
		(sensor_info->extra_mode & EXT_MODE) == SNX3C_60 ||
		(sensor_info->extra_mode & EXT_MODE) == SNX3C_100) {
...
int32_t sensor_x3c_linear_init(sensor_info_t *sensor_info)
{
	...
	vin_info("sensor_x3c_linear_init OK!\n");
	return ret;
}
...
int32_t sensor_x3c_pwl_init(sensor_info_t *sensor_info)
{
	...
	if ((extra_mode[sensor_info->port] & 0xf)== SNX3C_60)
		vin_info("The pre awb ratio is OX3GB-O060+038\n");
	else if ((extra_mode[sensor_info->port] & 0xf) == OF_OVX3C_24M)
		vin_info("The pre awb ratio is OX3GB-A100");
	else
...
int32_t sensor_mode_config_init(sensor_info_t *sensor_info)
{
	...
	if (sensor_info->config_index & TEST_PATTERN) {
		vin_info("ov_test_pattern 0x%04x\n", ov_test_pattern[1]);
...
int sensor_ovx3c_serializer_init(sensor_info_t *sensor_info)
{
	...
	if ((sensor_info->extra_mode & 0xff) == LCEX3C_60 ||
		(sensor_info->extra_mode & 0xff) == LCEX3C_100) {
...
int32_t sensor_init(sensor_info_t *sensor_info)
{
	...
	if (deserial_if && (!strcmp(deserial_if->deserial_name, "max96712") ||
		...
		if (sensor_info->extra_mode == SENSING_27M ||
			sensor_info->extra_mode == PHENIX_24M ||
			sensor_info->extra_mode == GAX3C ||
			sensor_info->extra_mode == LCEX3C_60 ||
			sensor_info->extra_mode == LCEX3C_100 ||
			sensor_info->extra_mode == SNX3C_60 ||
			sensor_info->extra_mode == SNX3C_100) {
	...
	if ((sensor_info->config_index & TRIG_STANDARD) ||
		...
		if (((sensor_info->extra_mode & EXT_MODE) == SENSING_27M) ||
		...
		} else if ((sensor_info->extra_mode & EXT_MODE) == LCEX3C_60 ||
			(sensor_info->extra_mode & EXT_MODE) == LCEX3C_100) {
			...
			if (ret < 0) {
				vin_err("write max9295_max96717_trigger_mfp0 error\n");
			}
		}
	}
...
int32_t sensor_stop(sensor_info_t *sensor_info)
{
	...
	if (deserial_if) {
		if (sensor_info->config_index & DES_STREAMOFF) {
			ret1 = sensor_serdes_stream_off(sensor_info);
			if (ret1 < 0) {
		...
		uint32_t grgain, uint32_t gbgain, uint32_t color_temper)
{
	...
	if (config_index[port] & AWB_CTRL_RATIO_DISABLE) {
		awb_alight[0] = 1;
		awb_alight[1] = 1;
		awb_cwf[0] = 1;
		awb_cwf[1] = 1;
		awb_d65[0] = 1;
		awb_d65[1] = 1;
	} else if ((extra_mode[port] & 0xf) == OF_OVX3C_24M) {
		awb_alight[0] =
	...
	} else if ((extra_mode[port] & 0xf) == SNX3C_60) {
		awb_alight[0] =
	...
	} else if ((extra_mode[port] & 0xf) == LCEX3C_100) {
		awb_alight[0] =
	...
	} else if ((extra_mode[port] & 0xf) == LCEX3C_60) {
		awb_alight[0] =
		...
		uint32_t *line, uint32_t line_num, uint32_t *again, uint32_t *dgain, uint32_t gain_num)
{
	...
	if (name_2a_thread_once[port]) {
		snprintf(tname, sizeof(tname), "sen_2a_%d", port);
	...
	if (((extra_mode[port] & AE_CTRL_MODE_MASK) == SNX3C_60) ||
		((extra_mode[port] & AE_CTRL_MODE_MASK) == LCEX3C_60)) {
		lcg_spd_ratio = OVX3C_LCG_SPD_RATIO_SN60;
		lcg_vs_ratio = OVX3C_LCG_VS_RATIO_SN60;
	}
	if ((extra_mode[port] & AE_CTRL_MODE_MASK) == LCEX3C_100) {
...
int32_t write_ae_reg(hal_control_info_t *info) {
	...
	ret = vin_write_array(info->bus_num, info->sensor_addr,
		SENSOR_REG_WIDTH, setting_size, ae_reg_array[info->port]);
	if (ret < 0) {
