...
* All rights reserved.
***************************************************************************/
#define pr_fmt(fmt)		"[ov10635_ix019]:" fmt
...
int sensor_ov10635_ix019_954_init(sensor_info_t *sensor_info)
{
	int ret = RET_OK;
	int setting_size = 0;
	...
	/*======954init======*/
	setting_size = sizeof(ds954_ov10635_ix019_init_setting)/sizeof(uint32_t)/2;
	...
	ret = vin_write_array(deserial_if->bus_num, deserial_if->deserial_addr, 1, setting_size, ds954_ov10635_ix019_init_setting);
	if (ret < 0) {
...
int sensor_init(sensor_info_t *sensor_info)
{
	...
	/*======954init======*/
	ret = sensor_ov10635_ix019_954_init(sensor_info);
	if (ret < 0) {
		vin_err("sensor_ov10635_ix019_954_init fail\n");
		return ret;
	}
	/*======913init======*/
	...
	ret = vin_write_array(sensor_info->bus_num, sensor_info->sensor_addr, 2, setting_size, ov10635_init_setting);
	if (ret < 0) {
...
int sensor_start(sensor_info_t *sensor_info)
{
	...
	if (ret < 0) {
	 	 vin_err("ov10635_ix019 s954 stream_on fail\n");
...
int sensor_stop(sensor_info_t *sensor_info)
{
	...
	deserial_if = (deserial_info_t *)sensor_info->deserial_info;
	ret = ((deserial_module_t *)(deserial_if->deserial_ops))->stream_off(deserial_if, sensor_info->deserial_port);
	if (ret < 0) {
	 	 vin_err("ov10635_ix019 s954 stream_off fail\n");
...
sensor_module_t ov10635_ix019 = {
	.module = "ov10635_ix019",
	.init = sensor_init,
	.start = sensor_start,
	.stop = sensor_stop,
