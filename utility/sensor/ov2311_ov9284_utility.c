...
* All rights reserved.
***************************************************************************/
#define pr_fmt(fmt)		"[ov2311_ov9284]:" fmt
...
#include "inc/ov2311_ov9284_setting.h"
...
int sensor_poweron(sensor_info_t *sensor_info) {
  ...
  ret = hb_vin_i2c_write_reg16_data8(deserial_if->bus_num,
                                 ov2311_ov9284_power_on_setting[1] >> 1,
                                 ov2311_ov9284_power_on_setting[3],
                                 ov2311_ov9284_power_on_setting[4]);
  if (ret < 0) {
...
int sensor_poweroff(sensor_info_t *sensor_info) {
  ...
  if (sensor_info->port != 0)
	  return ret;
  ret = hb_vin_i2c_write_reg16_data8(deserial_if->bus_num,
                                      ov2311_ov9284_power_off_setting[1] >> 1,
                                      ov2311_ov9284_power_off_setting[3],
                                      ov2311_ov9284_power_off_setting[4]);

  vin_err("sensor_poweroff \n");
  if (ret < 0) {
...
int sensor_init(sensor_info_t *sensor_info) {
  ...
  if (ret < 0) {
    vin_err("poweron or poweroff %s fail\n", sensor_info->sensor_name);
    return ret;
  }
  /*======max9296a_max9295a_max9295a======*/
  pdata = ov2311_ov9284_init_setting;
  setting_size = sizeof(ov2311_ov9284_init_setting) / sizeof(uint8_t);
...
int sensor_start(sensor_info_t *sensor_info) {
  ...
  pdata = ov2311_ov9284_stream_on_setting;
  setting_size = sizeof(ov2311_ov9284_stream_on_setting) / sizeof(uint8_t);
  ret = write_register(bus, pdata, setting_size);

  if (ret < 0) {
    vin_err("ov2311_ov9284 max9296a stream_on fail\n");
    return -1;
  }
  vin_err("ov2311_ov9284 max9296a stream_on success\n");
  return ret;
}
...
int sensor_stop(sensor_info_t *sensor_info) {
  ...
  pdata = ov2311_ov9284_stream_off_setting;
  setting_size = sizeof(ov2311_ov9284_stream_off_setting) / sizeof(uint8_t);
  ret = write_register(bus, pdata, setting_size);

  if (ret < 0) {
    vin_err("ov2311_ov9284 max9296a stream_off fail\n");
    return -2;
  }
  vin_err("ov2311_ov9284 max9296a stream_off success\n");
  return ret;
}
...
sensor_module_t ov2311_ov9284 = {
    .module = "ov2311_ov9284",
    .init = sensor_init,
    .start = sensor_start,
    .stop = sensor_stop,
