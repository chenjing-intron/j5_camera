...
static int32_t sensor_spi_write(sensor_info_t *sensor_info,
			uint32_t reg_addr, char *buffer, uint32_t sizee)
{
	...
	ret = vin_spi_write_block(sensor_info->bus_num, senbuf, sizee + 2);
	if(ret < 0) {
		free(senbuf);
		vin_err("malloc fail\n");
		return -RET_ERROR;
...
static int32_t sensor_spi_read(sensor_info_t *sensor_info,
				uint32_t reg_addr, char *buffer, uint32_t sizee)
{
	int32_t ret = RET_OK;
	...
	ret = vin_spi_read_block(sensor_info->bus_num, revbuf, sizee + 2);
	if(ret) {
		free(revbuf);
		vin_err("malloc fail\n");
		return -RET_ERROR;
			...
			sensor_turning_data_t *turning_data)
{
	...
	fps = sensor_info->fps;
	ret = hb_vin_i2c_read_block_reg16(sensor_info->bus_num, sensor_info->sensor_addr, IMX327_VAMX, init_d, 3);
	...
	ret = hb_vin_i2c_read_block_reg16(sensor_info->bus_num, sensor_info->sensor_addr, IMX327_HAMX, init_d, 2);
	...
	vin_info("exposure_time_max %d\n", turning_data->sensor_data.exposure_time_max);
	vin_info("exposure_time_long_max %d\n", turning_data->sensor_data.exposure_time_long_max);
	turning_data->sensor_data.fps = sensor_info->fps;
	ret = hb_vin_i2c_read_block_reg16(sensor_info->bus_num, sensor_info->sensor_addr, IMX327_RHS1, init_d, 3);
	...
	ret = hb_vin_i2c_read_block_reg16(sensor_info->bus_num, sensor_info->sensor_addr, IMX327_RHS2, init_d, 3);
	...
	ret = hb_vin_i2c_read_block_reg16(sensor_info->bus_num, sensor_info->sensor_addr, IMX327_CSI_LANE_MODE, init_d, 1);
	if (init_d[0] == 3)
...
int sensor_dol2_update_notify_driver(sensor_info_t *sensor_info)
{
	...
	vin_info("sensor_dol2_update_notify_driver\n");
	vin_info("sensor_info fps %d\n\n", sensor_info->fps);
#ifdef TUNING_LUT
	turning_data.dol2.line_p[0].ratio = -256;
	turning_data.dol2.line_p[0].offset = turning_data.sensor_data.RHS1 - 1;
...
int sensor_update_fps_notify_driver(sensor_info_t *sensor_info)
{
		...
		switch(sensor_info->sensor_mode) {
			case NORMAL_M:  //  normal
				...
				if (ret < 0) {
					vin_err("sensor_normal_update_notify_driver fail\n");
					return ret;
				}
				break;
			case DOL2_M:
...
static int sensor_dynamic_switch_fps(sensor_info_t *sensor_info, uint32_t fps)
{
	...
	switch (fps) {
		case IMX327_10FPS:
			vin_info("IMX327_10FPS\n");
			buf[0] = 0x01;
		    ret = hb_vin_i2c_write_block_reg16(sensor_info->bus_num, sensor_info->sensor_addr,
			...
			if(ret < 0) {
				vin_err("sensor_10fps_dynamic_switch fail\n");
				return -RET_ERROR;
			}
			buf[0] = 0x00;
			ret = hb_vin_i2c_write_block_reg16(sensor_info->bus_num, sensor_info->sensor_addr,
		...
		case IMX327_15FPS:
			...
			if(ret < 0) {
				vin_err("sensor_15fps_dynamic_switch fail\n");
				return -RET_ERROR;
			}
			buf[0] = 0x00;
			ret = hb_vin_i2c_write_block_reg16(sensor_info->bus_num, sensor_info->sensor_addr,
		...
		case IMX327_25FPS:
			...
			if(ret < 0) {
				vin_err("sensor_25fps_dynamic_switch fail\n");
				return -RET_ERROR;
			}
			buf[0] = 0x00;
			ret = hb_vin_i2c_write_block_reg16(sensor_info->bus_num, sensor_info->sensor_addr,
			...
			break;
		case IMX327_30FPS:
			vin_info("IMX327_30FPS\n");
			buf[0] = 0x01;
		    ret = hb_vin_i2c_write_block_reg16(sensor_info->bus_num,
			...
			if(ret < 0) {
				vin_err("sensor_30fps_dynamic_switch fail\n");
				return -RET_ERROR;
			}
			buf[0] = 0x00;
			ret = hb_vin_i2c_write_block_reg16(sensor_info->bus_num,
...
static int sensor_new_config_func(sensor_info_t *sensor_info, uint32_t *imx327_config, int size)
{
	...
	if(sensor_info->bus_type == I2C_BUS) {
		ret = vin_write_array(sensor_info->bus_num, sensor_info->sensor_addr, 2, setting_size, imx327_config);
		if (ret < 0) {
			vin_err("vin_write_array %s fail\n", sensor_info->sensor_name);
			return ret;
		}
		...
		for(i = 0; i < setting_size; i++) {
			...
			if (ret < 0) {
				vin_err("sensor_spi_write %s fail\n", sensor_info->sensor_name);
				return ret;
			}
		}
	}
...
int sensor_imx327_normal_init(sensor_info_t *sensor_info)
{
	...
	vin_info("imx327_linear_config %dfps_%dP success\n", fps, resolution);
	return ret;
}
int sensor_imx327_dol2_init(sensor_info_t *sensor_info)
{
	...
	if (req != 0) {
		vin_info("sensor_%s has alrady inited\n", sensor_info->sensor_name);
		return RET_OK;
	}
	...
	switch (fps) {
		case IMX327_25FPS:
			...
			vin_info("imx327 dol2 25fps init success req %d\n",req);
			break;
		case IMX327_30FPS:
			if(resolution == 2228) {
			...
			vin_info("imx327 dol2 30fps init success req %d\n", req);
			break;
		default:
			vin_err("not support fps type %d\n", fps);
			ret = -RET_ERROR;
...
int sensor_imx327_dol3_init(sensor_info_t *sensor_info)
{
	...
	switch (fps) {
		case IMX327_15FPS:
			...
			vin_info("imx327 dol3 15fps init success req %d\n", req);
			break;
		default:
			vin_err("not support fps type %d\n", fps);
			ret = -RET_ERROR;
...
static int sensor_init(sensor_info_t *sensor_info)
{
	...
	switch(sensor_info->sensor_mode) {
		case NORMAL_M:    //  normal
			...
			if(ret < 0) {
				vin_err("sensor_imx327_normal_init %s fail\n", sensor_info->sensor_name);
				return ret;
			}
		...
		case DOL2_M:  //  DOl2
			...
			if(ret < 0) {
				vin_err("sensor_imx327_dol2_init %s fail\n", sensor_info->sensor_name);
				return ret;
			}
			if(sensor_info->extra_mode == 0) {    //   dol2
				...
				if(ret < 0) {
					vin_err("sensor_dol2_long_frame_data_init %s fail\n",
						sensor_info->sensor_name);
					return ret;
				}
		...
		case DOL3_M:  //  DOl3
			...
			if(ret < 0) {
				vin_err("sensor_imx327_dol3_init %s fail\n", sensor_info->sensor_name);
				return ret;
			}
			if(sensor_info->extra_mode == 0) {     //   dol3
				...
				if(ret < 0) {
					vin_err("sensor_dol3_LEF_SEF1_data_init %s fail\n",
						sensor_info->sensor_name);
					return ret;
				}
			}
				...
				if(ret < 0) {
					vin_err("sensor_dol3_LEF_SEF2_data_init %s fail\n",
						sensor_info->sensor_name);
					return ret;
				}
				...
				if(ret < 0) {
					vin_err("sensor_dol3_SEF1_SEF2_data_init %s fail\n",
						sensor_info->sensor_name);
					return ret;
				}
			}
				...
				if(ret < 0) {
					vin_err("sensor_dol3_long_frame_data_init %s fail\n",
						sensor_info->sensor_name);
					return ret;
				}
				...
				if(ret < 0) {
					vin_err("sensor_dol2_short_frame_data_init %s fail\n",
						sensor_info->sensor_name);
					return ret;
				}
				...
				if(ret < 0) {
					vin_err("sensor_dol3_short_frame_data_init %s fail\n",
						sensor_info->sensor_name);
					return ret;
				}
			}
	...
	vin_info("imx327 config success under %d mode\n\n", sensor_info->sensor_mode);
	return ret;
}
...
static int sensor_start(sensor_info_t *sensor_info)
{
	...
	if(sensor_info->bus_type == I2C_BUS) {
		ret = vin_write_array(sensor_info->bus_num,
			sensor_info->sensor_addr, 2, setting_size, imx327_stream_on_setting);
		if(ret < 0) {
