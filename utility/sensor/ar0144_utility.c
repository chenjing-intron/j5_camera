...
int sensor_ar0144_954_init(sensor_info_t *sensor_info)
{
	int ret = RET_OK;
	int setting_size = 0;
	...
	/*======954init======*/
	setting_size = sizeof(ds954_ar0144_init_setting)/sizeof(uint32_t)/2;
	...
	ret = vin_write_array(deserial_if->bus_num, deserial_if->deserial_addr, 1, setting_size, ds954_ar0144_init_setting);
	if (ret < 0) {
...
int sensor_init(sensor_info_t *sensor_info)
{
	...
	setting_size = sizeof(ds933_ar0144_init_setting)/sizeof(uint32_t)/2;
	ret = vin_write_array(sensor_info->bus_num, sensor_info->serial_addr, 1, setting_size, ds933_ar0144_init_setting);
	if (ret < 0) {
...
int sensor_start(sensor_info_t *sensor_info)
{
	...
	if (ret < 0) {
	 	 vin_err("ar0144 s954 stream_on fail\n");
...
int sensor_stop(sensor_info_t *sensor_info)
{
	...
	if (ret < 0) {
	 	 vin_err("ar0144 s954 stream_off fail\n");
