...
int sensor_mmcam01_954_init(sensor_info_t *sensor_info)
{
	int ret = RET_OK;
	int setting_size = 0;
	...
	/*======954init======*/
	setting_size = sizeof(ds954_mult_init_setting)/sizeof(uint32_t)/2;
	...
	ret = vin_write_array(deserial_if->bus_num, deserial_if->deserial_addr, 1, setting_size, ds954_mult_init_setting);
	if (ret < 0) {
...
int sensor_start(sensor_info_t *sensor_info)
{
	...
	if (ret < 0) {
	 	 vin_err("mmcam01 s954 stream_on fail\n");
...
int sensor_stop(sensor_info_t *sensor_info)
{
	...
	ret = ((deserial_module_t *)(deserial_if->deserial_ops))->stream_off(
						deserial_if, sensor_info->deserial_port);
	if (ret < 0) {
	 	 vin_err("mmcam01 s954 stream_off fail\n");
