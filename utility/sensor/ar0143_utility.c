...
int sensor_ar0143_954_init(sensor_info_t *sensor_info)
{
	int ret = RET_OK;
	int setting_size = 0;
	...
	/*======954init======*/
	setting_size = sizeof(ds954_ar0143_init_setting)/sizeof(uint32_t)/2;
	...
	ret = vin_write_array(deserial_if->bus_num, deserial_if->deserial_addr, 1, setting_size, ds954_ar0143_init_setting);
	if (ret < 0) {
...
int sensor_init(sensor_info_t *sensor_info)
{
	...
	/*======954init======*/
	ret = sensor_ar0143_954_init(sensor_info);
	if (ret < 0) {
		vin_err("sensor_ar0143_954_init fail\n");
		return ret;
	}
	/*======933init======*/
...
int sensor_start(sensor_info_t *sensor_info)
{
	...
	if (ret < 0) {
	 	 vin_err("ar0143 s954 stream_on fail\n");
...
int sensor_stop(sensor_info_t *sensor_info)
{
	...
	if (ret < 0) {
	 	 vin_err("ar0143 s954 stream_off fail\n");
