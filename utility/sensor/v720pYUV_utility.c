...
* All rights reserved.
***************************************************************************/
#define pr_fmt(fmt)		"[v720pYUV]:" fmt
...
int sensor_v720pYUV_954_init(sensor_info_t *sensor_info)
{
	int ret = RET_OK;
	int setting_size = 0;
	...
	vin_write_array(deserial_if->bus_num, deserial_if->deserial_addr, 1,
		setting_size, ds954_ov10635_init_setting);
	usleep(500);
	vin_write_array(deserial_if->bus_num, deserial_if->deserial_addr, 1,
		setting_size, &(ds954_ov10635_init_setting[2]));
	usleep(300);
	setting_size = sizeof(ds954_ov10635_init_setting)/sizeof(uint32_t)/2 - 2;
	printf("setting_size = %d\n", setting_size);
	vin_write_array(deserial_if->bus_num, deserial_if->deserial_addr, 1,
		setting_size, &(ds954_ov10635_init_setting[4]));
	usleep(50000);
	if (ret < 0) {
		vin_err("write ds954_ov10635_init_setting error\n", sensor_info->serial_addr);
		return ret;
	}
	return ret;
}
...
int sensor_init(sensor_info_t *sensor_info)
{
	...
	/*======954init======*/
	ret = sensor_v720pYUV_954_init(sensor_info);
	if (ret < 0) {
		vin_err("sensor_v720pYUV_954_init fail\n");
		return ret;
	}
	/*======913init======*/
	setting_size = sizeof(ds913_ov10635_init_setting)/sizeof(uint32_t)/2;
	vin_write_array(sensor_info->bus_num, sensor_info->serial_addr, 1,
		setting_size, ds913_ov10635_init_setting);
	usleep(4000);
	return ret;
}
int sensor_start(sensor_info_t *sensor_info)
{
	...
	if (ret < 0) {
	  	 vin_err("v720pYUV 954 start fail\n");
