...
int32_t check_eeprom_wb_ratio(sensor_info_t *si) {
	...
	if ((data != 0xffff) && (data != 0x0) && ((data1 == 0x0) || (data1 == 0xffff))) {
		pr_debug("using sunny eeprom wb calib addr\n");
		...
		pr_debug("using lce eeprom wb calib addr\n");
		...
		pr_debug("no eeprom wb calib, using pre_awb\n");
	...
	if (checksum != data) {
		...
		checksum_flag[si->port] = 0;
		pr_debug("checksum error reg = %ld, cal = %ld, use pre_awb\n", data, checksum);
	} else {
...
int32_t deserial_serial_setting(sensor_info_t *sensor_info)
{
	...
	if (!strcmp(deserial_if->deserial_name, "max9296") ||
		...
		switch (sensor_info->config_index & 0xff) {
			...
			case SUNNY_NC120:
			case SUNNY_NC30:
				pdata = sunny_max96712_max96717_init_setting;
				setting_size = sizeof(sunny_max96712_max96717_init_setting)/sizeof(uint8_t);
				break;
	...
	if (!strcmp(deserial_if->deserial_name, "max96718")) {
		...
		if ((sensor_info->config_index & 0xff) == SENSING_X8B_WITH_X3C ||
			...
			(sensor_info->config_index & 0xff) == LCE_X3C_WITH_X8B ||
			(sensor_info->config_index & 0xff) == SUNNY_X8B_WITH_X3C_DPLL) {  // max96718 DUAL
